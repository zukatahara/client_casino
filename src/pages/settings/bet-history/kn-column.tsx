...
const getTheLoai = (theloai: string) => {
  return config().listKeno[theloai as "60"];
};

export const columnsKN: ColumnDef<BetLotteryHistory>[] = [
  {
  ...
  },
  {
    ...
    cell: ({ row }: { row: any }) => {
      ...
      return (
        <div>
          {row.original?.so
            ? config().listCuaDatKeno?.[row.original.type as "keo_doi"]?.[
                row.original.so as "chan"
              ]
            : ""}
        </div>
      );
    ...
    cell: ({ row }: { row: any }) => {
      return (
        <div className="text-yellow-500">
          {formatCurrency(row.original.cuoc || 0)}
        </div>
      );
    },
  },
